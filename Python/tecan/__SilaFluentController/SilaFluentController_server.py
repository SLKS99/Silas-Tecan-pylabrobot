...
*SilaFluentController*

:details: SilaFluentController:
    This is a python client for Fluent
           
:file:    SilaFluentController_server.py
:authors: Clemens Klein

:date: (creation)          2021-04-29T07:07:30.676111
:date: (last modification) 2021-04-29T07:07:30.676111
...
from SilaFluentController.SilaFluentController_servicer import SilaFluentController

logger = logging.getLogger(__name__)

class SilaFluentControllerServer(SiLA2Server):
    """
    This is a python client for Fluent
    """
    ...
    def __init__(self, cmd_args, simulation_mode: bool = True):
        ...
        #  Register SilaFluentController
        self.SilaFluentController_servicer = SilaFluentController(simulation_mode=self.simulation_mode)
        SilaFluentController_pb2_grpc.add_SilaFluentControllerServicer_to_server(
            self.SilaFluentController_servicer,
            self.grpc_server
        )
        self.add_feature(feature_id='SilaFluentController',
                         servicer=self.SilaFluentController_servicer,
                         data_path='meta')
...
def parse_command_line():
    ...
    """
    parser = argparse.ArgumentParser(description="A SiLA2 service: SilaFluentController")

    # Simple arguments for the server identification
    parser.add_argument('-s', '--server-name', action='store',
                        default="SilaFluentController", help='start SiLA server with [server-name]')
    parser.add_argument('-t', '--server-type', action='store',
                        default="Unknown Type", help='start SiLA server with [server-type]')
    parser.add_argument('-d', '--description', action='store',
                        default="This is a python client for Fluent", help='SiLA server description')
...
if __name__ == '__main__':
    ...
    sila_server = SilaFluentControllerServer(cmd_args=args, simulation_mode=True)
